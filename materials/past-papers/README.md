...
- 旧年份预览题草稿当前为 24 套、360 题，其中已通过华夏大地教育网公开页交叉补齐 2021 年 10 月近代史预览题 15 道答案；来源记录在 `index/public-answer-overrides.json`。
...
- 已整理 `index/zikaosw-answer-queue.json`：包含 2016-2021 马原/近代史预览题共 360 个答案入口。后续如果登录账号具备搜题包/答案查看次数，可用 `tools/fetch-zikaosw-answers.mjs` 分批抓取答案；脚本会合并已有结果，避免分批运行时覆盖旧数据。
- 新增 `index/zikaosw-access-check.json`：记录本次登录态验证结果，不保存 Cookie、账号或其他敏感信息。
- 新增 `index/public-answer-overrides.json`：记录从公开页面交叉验证到的答案，目前包含 2021 年 10 月近代史前 15 道预览题答案。
...
- 旧年份要做成完整可刷题库，还需要继续找完整 PDF，或开通搜题包/补充可用次数后抓答案，再做清洗。
...
- 华夏大地教育网公开真题页：保存了 2021 年 10 月近代史真题及答案页面，用于交叉补齐公开预览题答案。

## 后续处理建议

1. 优先解压 `raw/` 里的 ZIP/RAR，确认是否有重复卷。
2. 先处理已有 PDF/ZIP，它们大多自带答案，适合优先转成可刷题。
3. 旧网页题文先按 `index/older-web-pages.json` 对齐题号和答案入口，开通搜题包/找到 PDF 后再补答案。
4. 先生成旧网页预览题结构化草稿：
   ```bash
